...
samples = [(p,1) for p in T[:20]] + [(p,0) for p in O[:20]]
...
for idx,(path,label) in enumerate(samples,1):
    try:
        ...
        print(f"[{idx:02d}/40] expected={'Tampered' if label==1 else 'Original'} predicted={'Tampered' if pred==1 else 'Original'} conf={conf:.3f}")
    except Exception as e:
        print(f"[{idx:02d}/40] error: {e}")
